...
'@
    ...
    Context "Set global and script variable values in module session state" {
...
'@
            $testCases = @(
                ## Assignment in language
                @{
                    Name = 'global variable by language'
                    SetupScript = '& $mo { $Global:globalVar = "global variable by language" }'
                    ExpectedOutput = "global variable by language;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by language - dot'
                    SetupScript = '. $mo { $scriptVar = "script variable by language - dot" }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by language - dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by language - &'
                    SetupScript = '& $mo { $Script:scriptVar = "script variable by language - &" }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by language - &;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## New-Variable
                @{
                    Name = "global variable by New-Variable"
                    SetupScript = '& $mo { New-Variable globalVar -Value "global variable by New-Variable" -Force -Scope Global }'
                    ExpectedOutput = "global variable by New-Variable;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = "script variable by New-Variable - dot"
                    SetupScript = '. $mo { New-Variable scriptVar -Value "script variable by New-Variable - dot" -Force }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by New-Variable - dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = "script variable by New-Variable - &"
                    SetupScript = '& $mo { New-Variable scriptVar -Value "script variable by New-Variable - &" -Force -Scope Script }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by New-Variable - &;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Set-Variable
                @{
                    Name = 'global variable by Set-Variable'
                    SetupScript = '& $mo { Set-Variable globalVar -Value "global variable by Set-Variable" -Force -Scope Global }'
                    ExpectedOutput = "global variable by Set-Variable;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by Set-Variable - dot'
                    SetupScript = '. $mo { Set-Variable scriptVar -Value "script variable by Set-Variable - dot" }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by Set-Variable - dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by Set-Variable - &'
                    SetupScript = '& $mo { Set-Variable scriptVar -Value "script variable by Set-Variable - &" -Scope Script }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by Set-Variable - &;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## New-Item
                @{
                    Name = 'script variable by New-Item'
                    SetupScript = '. $mo { New-Item variable:\scriptVar -Value "script variable by New-Item" -Force }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by New-Item;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Set-Item
                @{
                    Name = 'script variable by Set-Item'
                    SetupScript = '. $mo { Set-Item variable:\scriptVar -Value "script variable by Set-Item" -Force }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;script variable by Set-Item;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Error Variable
                @{
                    Name = 'global variable by ErrorVariable'
                    SetupScript = '& $mo { Write-Error "Error-global" -ErrorAction SilentlyContinue -ErrorVariable global:globalVar }'
                    ExpectedOutput = "Error-global;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by ErrorVariable - dot'
                    SetupScript = '. $mo { Write-Error "Error-script-dot" -ErrorAction SilentlyContinue -ErrorVariable scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Error-script-dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by ErrorVariable - &'
                    SetupScript = '& $mo { Write-Error "Error-script-&" -ErrorAction SilentlyContinue -ErrorVariable script:scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Error-script-&;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Out Variable
                @{
                    Name = 'global variable by OutVariable'
                    SetupScript = '& $mo { Write-Output "Out-global" -OutVariable global:globalVar }'
                    ExpectedOutput = "Out-global;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by OutVariable - dot'
                    SetupScript = '. $mo { Write-Output "Out-script-dot" -OutVariable scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Out-script-dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by OutVariable - &'
                    SetupScript = '& $mo { Write-Output "Out-script-&" -OutVariable script:scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Out-script-&;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Warning Variable
                @{
                    Name = 'global variable by WarningVariable'
                    SetupScript = '& $mo { Write-Warning "Warning-global" -WarningAction SilentlyContinue -WarningVariable global:globalVar }'
                    ExpectedOutput = "Warning-global;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by WarningVariable - dot'
                    SetupScript = '. $mo { Write-Warning "Warning-script-dot" -WarningAction SilentlyContinue -WarningVariable scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Warning-script-dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by WarningVariable - &'
                    SetupScript = '& $mo { Write-Warning "Warning-script-&" -WarningAction SilentlyContinue -WarningVariable script:scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Warning-script-&;ParameterArgumentValidationError,Test-Untrusted"
                }

                ## Information Variable
                @{
                    Name = 'global variable by InformationVariable'
                    SetupScript = '& $mo { Write-Information "Information-global" -InformationAction SilentlyContinue -InformationVariable global:globalVar }'
                    ExpectedOutput = "Information-global;ParameterArgumentValidationError,Test-Untrusted;Trusted-Script;Trusted-Script"
                },
                @{
                    Name = 'script variable by InformationVariable - dot'
                    SetupScript = '. $mo { Write-Information "Information-script-dot" -InformationAction SilentlyContinue -InformationVariable scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Information-script-dot;ParameterArgumentValidationError,Test-Untrusted"
                },
                @{
                    Name = 'script variable by InformationVariable - &'
                    SetupScript = '& $mo { Write-Information "Information-script-&" -InformationAction SilentlyContinue -InformationVariable script:scriptVar }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;Information-script-&;ParameterArgumentValidationError,Test-Untrusted"
                },

                ## Data Section
                <# @{
                    Name = 'script variable by Data Section - "data script:var"'
                    ## 'data script:var { }' syntax is not supported today. If it's added someday, this test should be enabled
                    SetupScript = '& $mo { data Script:scriptVar { "data section - [Script:]" } }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;data section - [Script:];ParameterArgumentValidationError,Test-Untrusted"
                }, #>
                @{
                    Name = 'script variable by Data Section'
                    SetupScript = '. $mo { data scriptVar { "data section" } }'
                    ExpectedOutput = "Trusted-Global;Trusted-Global;data section;ParameterArgumentValidationError,Test-Untrusted"
                }
            )
        }
        ...
        It "Enable 'data script:var' test if the syntax is supported" {
            try {
                [scriptblock]::Create('data script:var { "data section" }')
                throw "No Exception!"
            } catch {
                
                ## Syntax 'data script:var { }' is not supported at the time writting the tests here
                ## If this test fail, then maybe this syntax is supported now, and in that case, please
                ## enable the test 'script variable by Data Section - "data script:var"' in $testCases above
                $_.FullyQualifiedErrorId | Should Be "ParseException"
            }
        }
    }
...
'@
        ...
        It "test module script variable set by Import-LocalizedData" {
            $testScript = @'
Get-ScriptVar
try { Test-WithScriptVar } catch { $_.FullyQualifiedErrorId }
'@
            Execute-Script -Script ". `$mo { Import-LocalizedData -BindingVariable scriptVar -BaseDirectory $TestDrive -FileName local.psd1 }"
            $result = Execute-Script -Script $testScript
...
'@
        ...
        It "test script variable set by exported variable" {
            try {
                ## Import the module in the module scope using dot-sourcing, so only the
                ## script variable is affected, the global variable is not.
                Execute-Script -Script ". `$mo { Import-Module $VarModule }"
                $result = Execute-Script -Script $testScript

                $result -join ";" | Should Be "Trusted-Global;Trusted-Global;script-from-module;ParameterArgumentValidationError,Test-Untrusted"
            } finally {
                Execute-Script -Script '. $mo { Remove-Module Var -Force }'
            }
        }
    }
...
'@
        ...
        It "test splatting module script variable" {
            $testScript = @'
Get-ScriptVar
try { Test-SplatScriptVar } catch { $_.FullyQualifiedErrorId }
'@
            Execute-Script -Script '. $mo { $scriptVar = @("script-splatting") }'
            $result = Execute-Script -Script $testScript

            $result -join ";" | Should Be "script-splatting;ParameterArgumentValidationError,Test-Untrusted"
        }
    }
